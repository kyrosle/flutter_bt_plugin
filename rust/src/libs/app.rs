...
use crate::libs::{DownloadArgs, Result};
...
impl App {
  pub fn new(download_dir: PathBuf) -> Result<Self> {
    ...
    let (engine, alert_rx) = bt_rust::engine::spawn(conf)?;
  ...
  pub fn update_torrent_state(
    ...
    if let Some(torrent) = self.torrents.get_mut(&torrent_id) {
      // update file completion by checking which pieces were download
      // this round
      if let Some(pieces) = &stats.pieces.latest_completed {
        for piece_index in pieces.iter().cloned() {
          let piece_len = torrent.storage.piece_len(piece_index);
          let mut torrent_piece_offset =
            torrent.storage.torrent_piece_offset(piece_index);
          let mut consumed = 0;

          let file_range =
            torrent.storage.files_intersecting_piece(piece_index);
          let files = &mut torrent.files[file_range];
          for file in files.iter_mut() {
            let remaining_piece_len = piece_len as u64 - consumed;
            let file_slice = file
              .info
              .get_slice(torrent_piece_offset, remaining_piece_len);
            ...
            debug_assert!(
              ...
              "cannot have download more than file length"
            );
          }
        }
      }
      ...
      let latest_complete_pieces = if let (Some(mut existing), Some(new)) = (
      ...
      torrent.pieces = PieceStats {
        latest_completed: latest_complete_pieces,
        ..stats.pieces
      };
      ...
      if let Peers::Full(peers) = &mut torrent.peers {
        if is_seed {
          peers.sort_by(|a, b| {
            b.thruput.payload.up.rate.cmp(&a.thruput.payload.up.rate)
          });
        } else {
          peers.sort_by(|a, b| {
            b.thruput
      ...
      .iter_mut()
      {
        history.update(current, HISTORY_LIMIT);
      }
    }
  }
}
...
impl ThruputHistory {
  ...
  pub fn rate(&self) -> u64 {
    self.rates.iter().next_back().cloned().unwrap_or_default()
  }
}
